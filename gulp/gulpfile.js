...
var WORK_ROOT_PATH = './';
var DEV_PATH = WORK_ROOT_PATH +'src/';

var ARR_TASK_COMPILE = [];
var ARR_TASK_OPTIMIZE = [];

// define task : SCSS -> CSS & Build
var BUILD_PATH = WORK_ROOT_PATH + 'dist/';

// scss comfile task
gulp.task('scss:compile', scss_compile(DEV_PATH+'assets/'));

// build:  Asset Copy / Html url replace
ARR_TASK_OPTIMIZE.push('css:optimize');
ARR_TASK_OPTIMIZE.push('html:optimize') ;
ARR_TASK_OPTIMIZE.push('copy:assets') ;

// autoprefix and combine media queries task
gulp.task('css:optimize', css_optimize(DEV_PATH+'assets/', BUILD_PATH + 'assets/'));
gulp.task('html:optimize', url_replace_in_html(DEV_PATH, BUILD_PATH));
gulp.task('copy:assets', copy_assets(DEV_PATH+'assets/', BUILD_PATH + 'assets/'));
...
gulp.task('watch',
    ...
    function(){
        ...
        gulp.watch(DEV_PATH +'assets/scss/**/**/**/**/*.scss', ['scss:compile']);
        gulp.watch(DEV_PATH +'*.html').on('change', browserSync.reload);
    }
);
